...
## 2026-02-24｜投资执行规则（用户指定，长期记忆）
- 标的：标普500（按指数涨跌条件触发当月增持）。
- 规则：
  1) 若标普500在某日**单日跌幅达到 -1%**，则执行当月一笔**定额增持**。
  2) 若当月始终未触发规则1，则在**当月第三个星期五收盘前**，无论涨跌，执行当月一笔**定额增持**。
  3) 在满足规则1或规则2基础上，若当月出现**月跌幅达到 -5%**，则当月再执行一笔**定额增持**。
  4) 规则3触发并执行后，**当月不再增持**。
- 用户要求：该规则需长期记忆，并用于后续执行方案。
- 执行参数（后续默认）：
  - 交易标的：`SPY`
  - 单笔金额：`1000 USD`
  - 执行模式：**先确认再下单**（用户回复确认后执行）
  - API接入偏好：moomoo OpenAPI 先走 `SIM` 模拟环境，验证稳定后再考虑实盘。

## 2026-02-25｜会话容量高占用自动兜底（用户长期默认）
- 当主会话上下文接近上限（如 197k/200k）时，按以下策略自动执行：
  1) >92%：预警提示。
  2) >95%：自动输出关键状态压缩摘要。
  3) >98%：进入降载模式（短回复 + 关键操作优先），并建议切新会话继续。
- 用户确认：该策略设为长期默认。
